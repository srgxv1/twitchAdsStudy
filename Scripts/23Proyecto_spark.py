...
inputRDD = sc.textFile('23').map(lambda x: x.split('\t'))
...
df.groupBy(col("Game")).agg({"CurrentViewer":"sum"}).orderBy("sum(CurrentViewer)", ascending=False).limit(10).coalesce(1).write.format("com.databricks.spark.csv").save('out23')